...
happens(occurs(f(x), V2), 1):-
	initially(val(y, V1)),
	boolean_not(V1, V2), 
	assert(cached(happens(occurs(f(x), V2), 1))).

happens(occurs(f(y), V), 1):-
	initially(val(x, V)), 
	assert(cached(happens(occurs(f(y), V), 1))).

happens(occurs(f(x), V2), T):-
	holds(val(y, V1), T),
	boolean_not(V1,V2), 
	assert(cached(happens(occurs(f(x), V2), T))).

happens(occurs(f(y), V), T):-
	holds(val(x, V), T), 
	assert(cached(happens(occurs(f(y), V), T))).
...
happens(occurs(f(h), 0), T):-
	...
	neg(val(h, 2), T),
	has_value(antigen, 0, T),
	assert(cached(happens(occurs(f(h), 0), T))).
...
happens(occurs(f(h), 2), T):-
	...
	pos(val(h, 2), T), 
	assert(cached(happens(occurs(f(h), 2), T))).
...
happens(occurs(f(cII), 1), T):-
	...
	neg(val(cro, 3), T),
	pos(val(n, 1), T), 
	assert(cached(happens(occurs(f(cII), 1), T))).
...
happens(occurs(f(n), 1), T):-
	...
	neg(val(cI, 1), T),
	neg(val(cro, 2), T), 
	assert(cached(happens(occurs(f(n), 1), T))).

happens(occurs(f(n), 0), T):-
	%write('happens(occurs(f(n),0),'), write(T), write(').'), nl,
	pos(val(cI, 1), T),
	assert(cached(happens(occurs(f(n), 0), T))).
...
holds(val(X,V), Tk):-
	...
	assert(cached(holds(val(X, V), Tk))).
...
query(App, EndTime, LogFile):-
	...
	assert(endReasoningTime(EndTime)),
	appVars(App, Vars),
	getCPUtime(Tstart),
...
produceLogFileInits(App, EndTime, InitValsList, LogFile):-
	atom_concat('FLECresults/', App, LogFilePrefix0),
	atom_concat(LogFilePrefix0, '-', LogFilePrefix1),
	number_atom(EndTime, EndTimeStr),
...
produceTimesFile(App, EndTime, TimesFile):-
	%atom_concat('FLECresults/', App, TimesFilePrefix0),
