...
%-------- fishingTripStatus ------------------%
initiatedAt(fishingTripStatus(Vessel)=null, T1, -1, T2) :- 
    T1=<(1443650400), 1443650400 <T2.

initiatedAt(fishingTripStatus(Vessel)=aomni, T):-
    happensAt(start(anchoredOrMoored(Vessel)=true), T),
    holdsAt(fishingTripStatus(Vessel)=null, T).
initiatedAt(fishingTripStatus(Vessel)=aomni, T):-
    happensAt(start(anchoredOrMoored(Vessel)=true), T),
    holdsAt(fishingTripStatus(Vessel)=underWay, T).

initiatedAt(fishingTripStatus(Vessel)=aomi, T):-
    happensAt(start(anchoredOrMoored(Vessel)=true), T),
    holdsAt(fishingTripStatus(Vessel)=interrupted, T).


initiatedAt(fishingTripStatus(Vessel)=underWay, T):-
    happensAt(start(underWay(Vessel)=true), T),
    holdsAt(fishingTripStatus(Vessel)=null, T).
initiatedAt(fishingTripStatus(Vessel)=underWay, T):-
    happensAt(start(underWay(Vessel)=true), T),
    holdsAt(fishingTripStatus(Vessel)=aomni, T).
initiatedAt(fishingTripStatus(Vessel)=underWay, T):-
    happensAt(start(underWay(Vessel)=true), T),
    holdsAt(fishingTripStatus(Vessel)=aomi, T).
initiatedAt(fishingTripStatus(Vessel)=underWay, T):-
    happensAt(start(underWay(Vessel)=true), T),
    holdsAt(fishingTripStatus(Vessel)=trawling, T).

initiatedAt(fishingTripStatus(Vessel)=trawling, T):-
    happensAt(start(trawling(Vessel)=true), T),
    holdsAt(fishingTripStatus(Vessel)=underWay, T).

initiatedAt(fishingTripStatus(Vessel)=interrupted, T):-
    happensAt(start(gap(Vessel)=_status), T),
    holdsAt(fishingTripStatus(Vessel)=underWay, T).
initiatedAt(fishingTripStatus(Vessel)=interrupted, T):-
    happensAt(start(gap(Vessel)=_status), T),
    holdsAt(fishingTripStatus(Vessel)=trawling, T).
initiatedAt(fishingTripStatus(Vessel)=interrupted, T):-
    happensAt(start(loitering(Vessel)=true), T),
    holdsAt(fishingTripStatus(Vessel)=underWay, T).
