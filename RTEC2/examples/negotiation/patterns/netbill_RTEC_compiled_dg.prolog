initiatedAt(quote(_131139,_131140,_131141)=true, _131150, _131124, _131152) :-
     ...
     _131150=<_131124,
     _131124<_131152.
    %updateVariableTemp(computed_initiations_extensible, 1).
...
initiatedAt(obl(send_EPO(Cons,iServer,GD))=true, T1, T, T2) :-
     ...
     %cachingOrder2(Merch, contract(Merch,Cons,GD)=true),
     %startCyclic(Merch, contract(Merch,Cons,GD)=true, T1, T, T2).

initiatedAt(obl(send_EPO(Cons,iServer,GD))=false, T1, T, T2) :-
     initiatedAt(contract(_Merch, Cons, GD)=false, T1, T, T2).
     %cachingOrder2(Merch, contract(Merch,Cons,GD)=true),
     %endCyclic(Merch, contract(Merch, Cons, GD)=true, T1, T, T2).
