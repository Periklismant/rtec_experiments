...
event(request_quote(_,_,_)).			inputEntity(request_quote(_,_,_)).		index(request_quote(Ag,_,_), Ag).		%key(request_quote(_,_,_), 1). 
event(present_quote(_,_,_,_)).			inputEntity(present_quote(_,_,_,_)).	index(present_quote(Ag,_,_,_), Ag). 	%key(present_quote(_,_,_,_), 2).
event(accept_quote(_,_,_)).				inputEntity(accept_quote(_,_,_)).		index(accept_quote(Ag,_,_), Ag). 		%key(accept_quote(_,_,_), 3).
event(send_EPO(_,_,_,_)).				inputEntity(send_EPO(_,_,_,_)).			index(send_EPO(Ag,_,_,_), Ag). 			%key(send_EPO(_,_,_,_), 4).
event(send_goods(_,_,_,_,_)).			inputEntity(send_goods(_,_,_,_,_)).		index(send_goods(Ag,_,_,_,_), Ag). 		%key(send_goods(_,_,_,_), 5).



% all values of a simple fluent must be declared
simpleFluent(quote(_,_,_)=true).				outputEntity(quote(_,_,_)=true).				index(quote(Ag,_,_)=true, Ag).				%key(quote(_,_,_)=true, 6).
simpleFluent(quote(_,_,_)=false).				outputEntity(quote(_,_,_)=false).				index(quote(Ag,_,_)=false, Ag).				%key(quote(_,_,_)=false, 7).
simpleFluent(contract(_,_,_)=true).				outputEntity(contract(_,_,_)=true).				index(contract(Ag,_,_)=true, Ag).			%key(contract(_,_,_)=true, 8).
simpleFluent(contract(_,_,_)=false).			outputEntity(contract(_,_,_)=false).			index(contract(Ag,_,_)=false, Ag).			%key(contract(_,_,_)=false, 9).
simpleFluent(per(present_quote(_,_))=true).		outputEntity(per(present_quote(_,_))=true).		index(per(present_quote(Ag,_))=true, Ag).	%key(per(present_quote(_,_))=true, 10).
simpleFluent(per(present_quote(_,_))=false).	outputEntity(per(present_quote(_,_))=false).	index(per(present_quote(Ag,_))=false, Ag).	%key(per(present_quote(_,_))=false, 11).
simpleFluent(obl(send_EPO(_,_,_))=true).		outputEntity(obl(send_EPO(_,_,_))=true).		index(obl(send_EPO(Ag,_,_))=true, Ag).		%key(obl(send_EPO(_,_,_))=true, 12).
simpleFluent(obl(send_EPO(_,_,_))=false).		outputEntity(obl(send_EPO(_,_,_))=false).		index(obl(send_EPO(Ag,_,_))=false, Ag).		%key(obl(send_EPO(_,_,_))=false, 13).
simpleFluent(obl(send_goods(_,_,_))=true).		outputEntity(obl(send_goods(_,_,_))=true).		index(obl(send_goods(Ag,_,_))=true, Ag).	%key(obl(send_goods(_,_,_))=true, 14).
simpleFluent(obl(send_goods(_,_,_))=false).		outputEntity(obl(send_goods(_,_,_))=false).		index(obl(send_goods(Ag,_,_))=false, Ag).	%key(obl(send_goods(_,_,_))=false, 15).
simpleFluent(suspended(_,_)=true).				outputEntity(suspended(_,_)=true).				index(suspended(Ag,_)=true, Ag).			%key(suspended(_,_)=true, 16).
simpleFluent(suspended(_,_)=false).				outputEntity(suspended(_,_)=false).				index(suspended(Ag,_)=false, Ag).			%key(suspended(_,_)=false, 17).

sDFluent(pow(accept_quote(_,_,_))=true).		outputEntity(pow(accept_quote(_,_,_))=true).	index(pow(accept_quote(Ag,_,_))=true, Ag).	%key(pow(accept_quote(_,_,_))=true, 18).
