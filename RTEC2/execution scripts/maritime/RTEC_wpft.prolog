...
:- ['dynamicGrounding/dynamicGrounding.prolog'].
...
eventRecognition(QueryTime, WM, FPDTimes) :-
	...
	prepareCyclic,
        ...
        statistics(StatisticsFlag, [S1A,_T1A]),
	...
	findall((F=V,Duration), (maxDuration(F=V,_,Duration), deadlines1(F=V,Duration,InitTime)), _),
        statistics(StatisticsFlag,[S2A,_T2A]),
        SA is S2A - S1A,
	...
	% by combining cachingOrder/1, indexOf/2 and grounding/1
        findall((S,OE),(
		   ...
		   findall(OE, (cachingOrder2(Index,OE), processEntity(Index,OE,InitTime,QueryTime)), _),
                   ...
                   S is S2-S1%,
                   %write(OE),write(' recognition time (ms): '),writeln(S)
                  ),
        FTimes),
	% DEADLINES #2 CHANGE
        statistics(StatisticsFlag, [S1B,_T1B]),
	findall((F=V,Duration), (maxDuration(F=V,_,Duration), deadlines2(F=V,Duration,InitTime)), _),
        statistics(StatisticsFlag, [S2B,_T2B]),
        SB is S2B-S1B,
        SD is SA+SB,
        FPDTimes = [SD,FTimes],
