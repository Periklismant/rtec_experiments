...
continuousQueries(App, WM, Step) :-
	...
	% handleApplication(+Prolog, +App, -InputMode, -LogFile, -ResultsFile, +WM, +Step, -StartReasoningTime, -EndReasoningTime, -StreamOrderFlag, -DynamicGroundingFlag, -PreprocessingFlag, -ClockTick, -SDEBatch),
	handleApplication(Prolog, App, InputMode, LogFile, ResultsFile, WM, Step, StartReasoningTime, EndReasoningTime, StreamOrderFlag, DynamicGroundingFlag, PreprocessingFlag, ClockTick, SDEBatch),
	format("                                                                 
	...
	% initialise RTEC
	% initialiseRecognition(+StreamOrderFlag, +DynamicGroundingFlag, +PreprocessingFlag, +ClockTick),	
	initialiseRecognition(StreamOrderFlag, DynamicGroundingFlag, PreprocessingFlag, ClockTick),
	...
	querying(InputStreams, PointerPositions, StatisticsFlag, LogFileStream, ResultsFileStream, WM, Step, QueryTime, EndReasoningTime, [], RecTimes, [], InputList, ([],[],[]), (OutputListOutFVpairs,OutputListOutLI,OutputListOutLD), [], TimesPPF, SDEBatch),
	processTimesPF(TimesPPF, StatsPF),
	...
	% write stats per fluent
        format(LogFileStream,'~`-tStatistics per fluent~`-t~72|~n~n',[]),
        writeStatsPerFluent(LogFileStream , StatsPF),
	nl(LogFileStream), nl(LogFileStream),
...
querying(_InputStreams, _InputPointerPositions, _StatisticsFlag, _LogFileStream, _ResultsFileStream, _WM, Step, QueryTime, EndReasoningTime, RecTimes, RecTimes, InputList, InputList, OutputList, OutputList, TimesPPF, TimesPPF, _SDEBatch) :-
	PrevQueryTime is QueryTime-Step,
 	PrevQueryTime >= EndReasoningTime, !.

querying(InputStreams, InputPointerPositions, StatisticsFlag, LogFileStream, ResultsFileStream, WM, Step, QueryTime, EndReasoningTime, InitRecTime, RecTimes, InitInput, InputList, (InitOutputOutFVpairs,InitOutputOutLI,InitOutputOutLD), OutputList, PrevTimesPPF, FTimesPPF, SDEBatch) :-
	...
	% eventRecognition(+QueryTime, +WM, -TimesPF)
	...
	eventRecognition(QueryTime, WM, TimesPPF),
	...
	querying(InputStreams, NewInputPointerPositions, StatisticsFlag, LogFileStream, ResultsFileStream, WM, Step, NextQueryTime, EndReasoningTime, [S|InitRecTime], RecTimes, [InL|InitInput], InputList, ([OutFVpairs|InitOutputOutFVpairs],[OutLI|InitOutputOutLI],[OutLD|InitOutputOutLD]), OutputList, [TimesPPF|PrevTimesPPF], FTimesPPF, SDEBatch).
...
% processTimesPF(+TimesPPF, -StatsPF)
% Input: A list of lists of lists containing the processing times for
%		 (deadlines/cycles) (normal processing) and (deadlines/cycles)
% Output: A list containing tuples with ((FluentName, Arity, Value), AvgTime, StDTime)
processTimesPF(TimesPPF, StatsPF):-
	computeSumsInW(TimesPPF, TimesPF),
	computeFinalStats(TimesPF, StatsPF).

% computeSumsInW(+TimesPPF, -TimesPF)
% Input:
% Output: 
computeSumsInW([],[]).
computeSumsInW([TimesPPF|RestTimesPPF],[TimesPF|RestTimesPF]):-
	setof(A, L^member((A, L), TimesPPF), UniqueFluents),
	computeSumPerFluentInW(UniqueFluents, TimesPPF, TimesPF),
	computeSumsInW(RestTimesPPF, RestTimesPF).

% computeSumPerFluentInW(+UniqueFluents, +TimesPPF, -TimesPF)
%
%
computeSumPerFluentInW([], TimesPPF, []).
computeSumPerFluentInW([A|R], TimesPPF, [(A,T)|RC]):-
	findall(Ti, member((A,Ti), TimesPPF), Tis),
	sumlist(Tis,T),
	computeSumPerFluentInW(R, TimesPPF, RC).

% computeStatsPerFluent(+UniqueFluents, +TimesPPF, -StatsList)
%
%
computeStatsPerFluent([], TimesPPF, []).
computeStatsPerFluent([A|R], TimesPPF, [(A,(Len, Sum, Avg, Var, Dev))|RC]):-
	findall(Ti, member((A,Ti), TimesPPF), Tis),
	list_stats(Tis, Len, Sum, Avg, Var, Dev),
	writeln(Len),
	computeStatsPerFluent(R, TimesPPF, RC).


% computeFinalStats(+TimesPFW, -Stats)

computeFinalStats(TimesPFW, Stats):-
	append_lists(TimesPFW, AllTimes),
	setof(A, L^member((A, L), AllTimes), UniqueFluents),
	computeStatsPerFluent(UniqueFluents, AllTimes, Stats).

% append_lists(+A, -F)
% A is list of lists and F the result of the concatenation of list of A.
%
append_lists(A, F):- append_lists(A, [], F).
append_lists([], F, F).
append_lists([A|R], P, F):-
	append(A, P, Pn),
	append_lists(R, Pn, F).

% writeStatsPerFluent(+Stream, +StatsPF)
% Writes the statisttics per fluent in Stream
%
writeStatsPerFluent(_,[]).
writeStatsPerFluent(Stream, [((Fluent, Arity, Value), (Len, Sum, Avg, Var, Dev)) | R]):-
	format(Stream, '~w/~d/~w ~t ~28| (total/avg/std) (ms): (~3f/~3f/~3f) ~n',[Fluent, Arity, Value, Sum, Avg, Dev]),
	writeStatsPerFluent(Stream,R).
