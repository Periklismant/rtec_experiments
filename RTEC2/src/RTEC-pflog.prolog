...
:- dynamic collectIntervals/1, collectIntervals2/2, buildFromPoints/1, buildFromPoints2/2, cyclic/1, maxDuration/3, maxDurationUE/3, internalEntity/1, sDFluent/1.
...
eventRecognition(QueryTime, WM, TimesPF) :-
	...
	preProcessing(QueryTime),
        % REQUIRED FOR MEASURING TIME PER FLUENT (handleProlog has to be loaded!)
	handleProlog(Prolog, StatisticsFlag),
	% CYCLES #1 CHANGE
	prepareCyclic,
	% -------------- start measuring time here ----------
	% CYCLES & DEADLINES CHANGE
	findall(((FluentName,Arity,V), PTime), (
                    cachingOrder(F=V),
	...
	findall(((FluentName,Arity,V),S),(
                   cachingOrder(F=V),
                   statistics(StatisticsFlag, [S1,_T1]),
		   findall(F=V, 
			      (cachingOrder2(Index,F=V),
			       processEntity(Index,F=V,InitTime,QueryTime)
                              ),
                   _),
                   statistics(StatisticsFlag,[S2,_T2]), 
                   S is S2-S1,
	...
	append(DTimes, CDTimes,TimesPF1),
	append(TimesPF1, FTimes, TimesPF),
