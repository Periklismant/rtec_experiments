...
forget(InitTime) :-
	...
	%updateTime(forget_retractions_time,Time2forget,Time1forget),
	%findall(E, (happensAtIEIndexed(T, E), T=<InitTime, EntityForm=.. [happensAtIEIndexed, T, _], retractall(EntityForm)), _), %retract(happensAtIE(E, T))), _),
...
/*
forgetIndexed(EntityName, InitTime):-
	Entity=.. [EntityName, _T, _E],
	write(Entity), nl,
	Entity, write(Entity), nl, (Tcurr=<InitTime -> Entity2=.. [EntityName, Tcurr, _], write(Entity2), nl, retractall(Entity2), nl ; nl).
*/
...
forgetEntity(InitTime, EntityName, EventsPerTimepointThreshold):-
	...
	%write('About to measure time...'), nl, 
	getCPUtime(Time2forget_determine),
	updateTime(forget_determine_method_time,Time1forget_determine,Time2forget_determine),
	(
		% The default case of forget which retracts one by one.
		EventsPerTimepointThreshold = -1, !,
		write('Forget: Retract one by one...'), nl,
		%write(Events), nl, nl, nl,
		%maplist(retract, Events) %
		%write(EventsSorted), nl, nl, nl,
		%retractallinList(EventsSorted)
		...
		write('Forget: Retractall at each time-point...'), nl,
	...
	getCPUtime(Time2endforget),
	updateTime(forget_retractions_time,Time2forget_determine,Time2endforget).
...
compare_by_timepoint(<, happensAtIE(E1, T1), happensAtIE(E2, T2)):-
	(T1 < T2 ;
	 T1 == T2, E1 @< E2).
