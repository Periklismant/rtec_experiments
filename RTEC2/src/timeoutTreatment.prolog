...
initiatedAt(NewF=NewV, T1, T, T2) :-
	write('\tIn initiatedAt via deadlines for '), write(NewF=NewV), nl,
        maxDuration(F=V, NewF=NewV, Duration),
	...
	% initiatedAt incarnation for deadline fluents:
        ...
        write('\t'), write(NewF=NewV), write(' is initiated at '), write(T), nl.
...
% that is subject to deadlines takes place within the working memory	
dInitiatedAt(F=V, Duration, T1, T, T2) :-
	write('\tIn dInitiatedAt for '), write(F=V), nl,
	...
	T is EarlierT+Duration,
	write('\tBefore deadline conditions for '), write(F=V), nl,
...
startedBetween4(Index, F=V, T1, T2) :-
        cyclic(F=V),
	initiatedAtCyclic(Index, F=V, T1, _, T2), !.
	%initiatedAt(F=V, T1, _, T2), !.
