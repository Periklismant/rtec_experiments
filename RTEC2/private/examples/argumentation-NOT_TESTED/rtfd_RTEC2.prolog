...
---object and retract are written as object and rtract to avoid the clash with Prolog's keywords
...
happensAt(declare(DetRole, Role, Q), T) :-
...
initiatedAt(premise(Role, Q)=true, T) :-
	happensAt(claim(Role, Q), T),
...
initiatedAt(premise(Role, Q)=false, T) :-
	happensAt(rtract(Role, Q), T),
	\+ holdsAt(turn=determiner, T).
...
initiatedAt(winner=Role, T) :-
	happensAt(declare(DetRole, Role), T).

initiatedAt(winner=none, T) :-
...
holdsFor(pow(claim(Role, Q))=true, I) :-
	\+ Role=determiner,
	holdsFor(turn=Role, I).

holdsFor(pow(concede(Role, Q))=true, I) :-
	\+ Role=determiner,
	holdsFor(turn=Role, I).

holdsFor(pow(rtract(Role, Q))=true, I) :-
	\+ Role=determiner,
	holdsFor(turn=Role, I).

holdsFor(pow(deny(Role, Q))=true, I) :-
	\+ Role=determiner,
	holdsFor(turn=Role, I).

holdsFor(pow(objct(Role, Act))=true, I) :-
	\+ Role=determiner,
	holdsFor(invalidActionHappened=Act, I).



initiatedAt(pow(declare(determiner, proponent))=true, T) :-
	happensAt(dTimeout, T),
	holdsAt(topic=Q, T),
	holdsFor(accepts(proponent, Q)=true, T).

initiatedAt(pow(declare(determiner, opponent))=true, T) :-
	happensAt(dTimeout, T),
	holdsAt(topic=Q, T),
	holdsFor(accepts(proponent, Q)=false, T).

initiatedAt(pow(declare(determiner, proponent))=false, T) :-
	happensAt(endTimeout, T).

initiatedAt(pow(declare(determiner, opponent))=false,  T) :-
	happensAt(endTimeout, T).
