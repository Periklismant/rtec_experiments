...
% ---------------------------------compile startI-------------------------------------
compileConditions1(happensAt(startI(U),T), NewBody, Timespan, Cyclic) :-
	simpleFluent(U), indexOf(Index, U),
	(
	Timespan = [],
	NewBody = happensAtProcessedSimpleFluent(Index,startI(U),T)
	;
	Timespan = [T1, T2],
	NewBody = (happensAtProcessedSimpleFluent(Index,startI(U),T), T1=<T, T<T2) 
	), 
	!.
...
compileConditions1(happensAt(startI(U),T), NewBody, Timespan, Cyclic) :-
	...
	(
	Timespan = [],
	NewBody = happensAtProcessedIE(Index,startI(U),T)
	;
	Timespan = [T1, T2],
	NewBody = (happensAtProcessedIE(Index,startI(U),T), T1=<T, T<T2)
	), 
	!.
...
compileConditions1(happensAt(startI(U),T), NewBody, Timespan, Cyclic) :-
	...
	(
	Timespan = [],
	NewBody = happensAtProcessedSDFluent(Index,startI(U),T)
	;
	Timespan = [T1, T2],
	NewBody = (happensAtProcessedSDFluent(Index,startI(U),T), T1=<T, T<T2)
	), 
	!.
% ---------------------------------compile startI-------------------------------------
